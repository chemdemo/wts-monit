...
* @Last Modified time: 2015-04-07 12:08:12
*/
...
function sockDataHandle(msg) {
    ...
    try {
        ...
        this.sendMsg({cmd: 'client:error', code: 1, message: e.message || 'JSON parse error'});
        logger.error(e);
        return;
    }
    ...
    if(!cmd) {
        this.sendMsg({cmd: 'client:error', code: 2, message: 'param `cmd` required'});
        logger.warn('client id:%s has not assigned', cid);
        return;
    }

    if(!cid) {
        this.sendMsg({cmd: 'client:error', code: 3, message: 'param `clientId` required'});
        logger.warn('client id:%s has not assigned', cid);
        return;
    }

    if(cid !== this.id) {
        this.sendMsg({cmd: 'client:error', code: 4, message: 'param `clientId` error'});
        logger.warn('client id:%s has not assigned', cid);
        return;
    }
